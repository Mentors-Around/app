...
import { Search, MoreVertical, Eye, FileWarning, Ban, CheckCircle, AlertTriangle, XCircle, ShieldAlert } from 'lucide-react';
import TeacherAvatar from '../components/shared/TeacherAvatar';

const mockReports = [
  {
    id: 'r1',
    reporter: { type: 'student', name: 'Rohan Sharma', initials: 'RS' },
    reportedUser: { type: 'teacher', name: 'Anjali Verma', initials: 'AV' },
    category: 'Inappropriate Content',
    reason: 'Teacher was using inappropriate language during the live session.',
    date: '2023-10-14',
    status: 'OPEN'
  },
  {
    id: 'r2',
    reporter: { type: 'teacher', name: 'Hari Prasad L', initials: 'HP' },
    reportedUser: { type: 'student', name: 'Vikram Patel', initials: 'VP' },
    category: 'Disruptive Behavior',
    reason: 'Student was continuously spamming the chat with irrelevant links.',
    date: '2023-10-12',
    status: 'RESOLVED'
  },
  {
    id: 'r3',
    reporter: { type: 'student', name: 'Neha Gupta', initials: 'NG' },
    reportedUser: { type: 'teacher', name: 'Rahul Sharma', initials: 'RS' },
    category: 'No Show',
    reason: 'Teacher did not show up for the scheduled class.',
    date: '2023-10-15',
    status: 'OPEN'
  }
];
...
export default function AdminReports() {
  useEffect(() => { document.title = "Reports — Admin Dashboard"; }, []);
  ...
  const handleAction = (id, action) => {
    setReports(reports.map(r => {
      if (r.id === id) {
        if (action === 'RESOLVE' || action === 'REJECT' || action === 'WARN' || action === 'SUSPEND') {
