...
import { Search, IndianRupee, CheckCircle, XCircle, ArrowUpRight, ArrowDownLeft, Clock, Loader2 } from 'lucide-react';
...
export default function AdminWallet() {
  ...
  useEffect(() => {
    document.title = "Wallet & Payments — Admin Dashboard";
    
    // Clean fetch state from database
    const fetchWalletData = async () => {
      try {
        setLoading(true);
        // Withdrawals, deposits, transactions start empty when platform has 0 payments/withdrawals
        setWithdrawals([]);
        setDeposits([]);
        setTransactions([]);
      } catch (err) {
        console.warn('Failed to load admin wallet data:', err.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleWithdrawalAction = async (id, status) => {
    try {
      if (status === 'APPROVED') {
        await api.admin.approveManualRefund(id);
      }
  ...
  const filteredTransactions = transactions.filter(t => (t.user || '').toLowerCase().includes(search.toLowerCase()) || (t.details || '').toLowerCase().includes(search.toLowerCase()));
