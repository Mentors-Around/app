...
import { MessagesSquare, CheckCircle, Clock, XCircle, ChevronRight, AlertCircle, RefreshCw, Star, MessageSquare, CreditCard } from 'lucide-react';
...
const MyQueriesPage = () => {
  ...
  useEffect(() => {
    ...
    const loadData = () => {
      ...
      if (savedClassroom) {
        try { combinedQueries = [...combinedQueries, ...JSON.parse(savedClassroom)]; } catch (e) {}
      }
      
      const myQueries = combinedQueries
        .filter(q => q.studentId === (user?.id || 'student-1'))
        .map(q => {
          if (q.status === 'approved_waiting_payment') {
            const timer = getPaymentTimer(q.paymentDeadline);
            if (timer.expired) {
              return { ...q, status: 'approval_expired' };
            }
          }
          return q;
        })
