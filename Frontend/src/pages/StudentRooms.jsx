import { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { PlayCircle, Eye, Download, Award, Calendar, BookOpen, Monitor, Sparkles, CheckCircle, Star, Loader2 } from 'lucide-react';
import StudentReviewModal from '../components/shared/StudentReviewModal';
import api from '../services/api';

const mockActiveClassrooms = [
  {
    id: '101',
    name: 'Mastering Calculus & Advanced Mathematics',
    teacher: 'Dr. Anand',
    subject: 'Mathematics',
    progress: 70,
    nextLiveClass: 'Today • 6:00 PM',
    attendance: 95,
    badges: ['Live Today', 'New Material']
  },
  {
    id: '102',
    name: 'Physics Crash Course for JEE',
    teacher: 'Sarah Smith',
    subject: 'Physics',
    progress: 45,
    nextLiveClass: 'Oct 15 • 4:00 PM',
    attendance: 80,
    badges: ['Upcoming']
  }
];

const mockCompletedClassrooms = [
  {
    id: '103',
    name: 'Introduction to Organic Chemistry',
    teacher: 'Rahul Sharma',
    teacherId: '1', // Added teacherId
    subject: 'Chemistry',
    completionDate: 'Sep 28, 2026',
    progress: 100,
    certificateAvailable: true
  },
  {
    id: '104',
    name: 'Basic English Grammar and Composition',
    teacher: 'Sneha R',
    teacherId: '2', // Added teacherId
    subject: 'English',
    completionDate: 'Aug 15, 2026',
    progress: 100,
    certificateAvailable: false
  }
];

const getBadgeStyle = (badge) => {
  switch (badge.toLowerCase()) {
    case 'live today':
      return 'bg-gradient-to-r from-red-500 to-rose-600 text-white shadow-sm';
    case 'upcoming':
      return 'bg-gradient-to-r from-amber-400 to-amber-500 text-white shadow-sm';
    case 'new material':
      return 'bg-gradient-to-r from-sky-400 to-blue-500 text-white shadow-sm';
    case 'certificate available':
      return 'bg-gradient-to-r from-emerald-400 to-green-500 text-white shadow-sm';
    default:
      return 'bg-slate-100 text-slate-700 border border-slate-200';
  }
};

const getBadgeIcon = (badge) => {
  switch (badge.toLowerCase()) {
    case 'live today': return <span className="animate-pulse w-2 h-2 bg-white rounded-full mr-1 inline-block"></span>;
    case 'upcoming': return '📅 ';
    case 'new material': return '✨ ';
    case 'certificate available': return '🏆 ';
    default: return '';
  }
};

export default function StudentRooms() {
  const [activeTab, setActiveTab] = useState('active');
  const [enrollments, setEnrollments] = useState([]);
  const [loading, setLoading] = useState(true);
  
  // Review System State
  const [submittedReviews, setSubmittedReviews] = useState({});
  const [reviewModalOpen, setReviewModalOpen] = useState(false);
  const [reviewContext, setReviewContext] = useState(null);

  useEffect(() => {
    document.title = 'My Learning — TrueEd';
    window.scrollTo(0, 0);
    fetchMyEnrollments();
  }, []);

  const fetchMyEnrollments = async () => {
    try {
      setLoading(true);
      const res = await api.user.getSavedClassrooms().catch(() => []);
      const list = Array.isArray(res) ? res : (res?.docs || res?.classrooms || []);
      setEnrollments(list);
    } catch (err) {
      console.error('Failed to load enrollments:', err);
    } finally {
      setLoading(false);
    }
  };

  const loadReviews = () => {
    const raw = localStorage.getItem('trueed_reviews');
    if (raw) {
      const allReviews = JSON.parse(raw);
      // Create a map of classroomId -> review for quick lookup
      const myReviewsMap = {};
      allReviews.forEach(r => {
        // Assume studentId === 'current_student' for this frontend mock
        if (r.studentId === 'current_student') {
          myReviewsMap[r.classroomId] = r;
        }
      });
      setSubmittedReviews(myReviewsMap);
    }
  };

  const handleOpenReview = (classroom) => {
    setReviewContext(classroom);
    setReviewModalOpen(true);
  };

  const handleSubmitReview = (reviewData) => {
    const raw = localStorage.getItem('trueed_reviews');
    let allReviews = raw ? JSON.parse(raw) : [];

    // Check if we are updating an existing review
    const existingIndex = allReviews.findIndex(r => r.classroomId === reviewContext.id && r.studentId === 'current_student');
    
    if (existingIndex >= 0) {
      // Update
      allReviews[existingIndex] = {
        ...allReviews[existingIndex],
        ...reviewData,
        updatedAt: new Date().toISOString()
      };
    } else {
      // Create new
      const newReview = {
        id: `REV-${Math.floor(Math.random() * 90000) + 10000}`,
        studentId: 'current_student',
        studentName: 'John Doe',
        teacherId: reviewContext.teacherId || '1',
        classroomId: reviewContext.id,
        classroomName: reviewContext.name,
        subject: reviewContext.subject,
        rating: reviewData.rating,
        text: reviewData.text,
        isAnonymous: reviewData.isAnonymous,
        createdAt: new Date().toISOString()
      };
      allReviews.push(newReview);
    }

    localStorage.setItem('trueed_reviews', JSON.stringify(allReviews));
    setReviewModalOpen(false);
    setReviewContext(null);
    loadReviews(); // Refresh local state
  };

  return (
    <div className="max-w-6xl mx-auto pb-12 space-y-8 animate-fadeIn">
      
      {/* Header & Tabs */}
      <div>
        <h1 className="font-sora text-3xl md:text-4xl font-extrabold text-navy mb-8">My Learning</h1>
        
        <div className="flex border-b border-slate-200 mb-8 overflow-x-auto hide-scrollbar">
          <button 
            onClick={() => setActiveTab('active')}
            className={`py-4 px-8 text-sm font-bold whitespace-nowrap transition-colors relative ${activeTab === 'active' ? 'text-navy' : 'text-slate-500 hover:text-navy'}`}
          >
            Active Classrooms
            {activeTab === 'active' && <div className="absolute bottom-0 left-0 w-full h-1 bg-navy rounded-t-full" />}
          </button>
          <button 
            onClick={() => setActiveTab('completed')}
            className={`py-4 px-8 text-sm font-bold whitespace-nowrap transition-colors relative ${activeTab === 'completed' ? 'text-navy' : 'text-slate-500 hover:text-navy'}`}
          >
            Completed Classrooms
            {activeTab === 'completed' && <div className="absolute bottom-0 left-0 w-full h-1 bg-navy rounded-t-full" />}
          </button>
        </div>
      </div>

      {/* Content Area */}
      <div className="animate-slide-up-sm">
        
        {/* Active Classrooms Tab */}
        {activeTab === 'active' && (
          mockActiveClassrooms.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {mockActiveClassrooms.map(room => (
                <div key={room.id} className="bg-white border border-slate-200 rounded-2xl p-6 hover:border-navy/20 transition-all shadow-sm flex flex-col h-full relative group">
                  
                  {room.badges && room.badges.length > 0 && (
                    <div className="absolute -top-3 -right-2 flex flex-col gap-1.5 items-end z-10">
                      {room.badges.map((badge, idx) => (
                        <span key={idx} className={`text-[10px] font-bold px-3 py-1.5 rounded-md uppercase tracking-wider flex items-center ${getBadgeStyle(badge)}`}>
                          {getBadgeIcon(badge)} {badge}
                        </span>
                      ))}
                    </div>
                  )}

                  <div className="mb-4">
                    <span className="text-[10px] font-bold bg-sky-50 text-sky-700 px-2.5 py-1 rounded-md uppercase tracking-wider mb-3 inline-block">
                      {room.subject}
                    </span>
                    <h3 className="font-sora font-bold text-xl text-navy mb-1 line-clamp-2 pr-8">{room.name}</h3>
                    <p className="text-sm font-medium text-slate-500">by {room.teacher}</p>
                  </div>

                  <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 mb-6 space-y-4 flex-1">
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Course Progress</span>
                        <span className="font-sora font-bold text-navy text-sm">{room.progress}%</span>
                      </div>
                      <div className="w-full bg-slate-200 rounded-full h-2">
                        <div className="bg-navy h-2 rounded-full transition-all" style={{ width: `${room.progress}%` }}></div>
                      </div>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-200/60">
                      <div>
                        <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Next Live Class</p>
                        <p className="text-sm font-bold text-slate-800 flex items-center gap-1.5">
                          <Calendar className="w-3.5 h-3.5 text-sky-500" /> {room.nextLiveClass}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Attendance</p>
                        <p className="text-sm font-bold text-slate-800 flex items-center gap-1.5">
                          <CheckCircle className="w-3.5 h-3.5 text-emerald-500" /> {room.attendance}%
                        </p>
                      </div>
                    </div>
                  </div>

                  <div className="flex flex-col sm:flex-row gap-3 mt-auto">
                    <Link to={`/classroom/${room.id}`} className="flex-1 py-3 bg-navy text-white font-bold text-sm text-center rounded-xl hover:bg-navy-light transition shadow-sm flex items-center justify-center gap-2">
                      <PlayCircle className="w-4 h-4" /> Continue Learning
                    </Link>
                    <Link to={`/classroom/${room.id}`} className="flex-1 py-3 bg-white border border-slate-200 text-slate-700 font-bold text-sm text-center rounded-xl hover:bg-slate-50 transition shadow-sm flex items-center justify-center gap-2">
                      <Eye className="w-4 h-4" /> View Classroom
                    </Link>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <div className="bg-white p-12 rounded-2xl border border-slate-200 text-center shadow-sm">
              <div className="w-20 h-20 bg-slate-50 text-slate-400 rounded-full flex items-center justify-center text-3xl mx-auto mb-6">
                <Monitor className="w-10 h-10" />
              </div>
              <h3 className="font-sora font-bold text-navy text-xl mb-3">You are not enrolled in any active classrooms yet</h3>
              <p className="text-slate-500 text-sm mb-8 max-w-sm mx-auto">Discover top teachers and enroll in classrooms to start your learning journey.</p>
              <Link to="/student/discover" className="inline-block px-8 py-3.5 bg-navy text-white rounded-xl font-bold shadow-sm hover:bg-navy-light transition">
                Discover Classrooms
              </Link>
            </div>
          )
        )}

        {/* Completed Classrooms Tab */}
        {activeTab === 'completed' && (
          mockCompletedClassrooms.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {mockCompletedClassrooms.map(room => (
                <div key={room.id} className="bg-white border border-slate-200 rounded-2xl p-6 hover:border-navy/20 transition-all shadow-sm flex flex-col h-full relative group">
                  
                  {room.certificateAvailable && (
                    <div className="absolute -top-3 -right-2 z-10">
                      <span className={`text-[10px] font-bold px-3 py-1.5 rounded-md uppercase tracking-wider flex items-center ${getBadgeStyle('certificate available')}`}>
                        {getBadgeIcon('certificate available')} Certificate Available
                      </span>
                    </div>
                  )}

                  <div className="mb-4">
                    <div className="flex justify-between items-start mb-3">
                      <span className="text-[10px] font-bold bg-slate-100 text-slate-600 px-2.5 py-1 rounded-md uppercase tracking-wider inline-block">
                        {room.subject}
                      </span>
                      <span className="text-[10px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-200 px-2.5 py-1 rounded-md uppercase tracking-wider flex items-center gap-1">
                        <CheckCircle className="w-3 h-3" /> Completed
                      </span>
                    </div>
                    <h3 className="font-sora font-bold text-xl text-slate-700 mb-1 line-clamp-2 pr-8">{room.name}</h3>
                    <p className="text-sm font-medium text-slate-500">by {room.teacher}</p>
                  </div>

                  <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 mb-6 space-y-4 flex-1">
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Final Progress</span>
                        <span className="font-sora font-bold text-emerald-600 text-sm">{room.progress}%</span>
                      </div>
                      <div className="w-full bg-slate-200 rounded-full h-2">
                        <div className="bg-emerald-500 h-2 rounded-full" style={{ width: `${room.progress}%` }}></div>
                      </div>
                    </div>
                    
                    <div className="pt-2 border-t border-slate-200/60">
                      <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Completion Date</p>
                      <p className="text-sm font-bold text-slate-800 flex items-center gap-1.5">
                        <Award className="w-4 h-4 text-amber-500" /> {room.completionDate}
                      </p>
                    </div>
                  </div>

                  <div className="flex flex-col gap-3 mt-auto">
                    <div className="flex flex-col sm:flex-row gap-3">
                      <Link to={`/classroom/${room.id}`} className="flex-1 py-3 bg-white border border-slate-200 text-slate-700 font-bold text-sm text-center rounded-xl hover:bg-slate-50 transition shadow-sm flex items-center justify-center gap-2">
                        <Eye className="w-4 h-4" /> View Classroom
                      </Link>
                      <Link to={`/classroom/${room.id}?tab=resources`} className="flex-1 py-3 bg-white border border-slate-200 text-slate-700 font-bold text-sm text-center rounded-xl hover:bg-slate-50 transition shadow-sm flex items-center justify-center gap-2">
                        <BookOpen className="w-4 h-4" /> View Resources
                      </Link>
                    </div>
                    {room.certificateAvailable && (
                      <button onClick={() => alert("Downloading Certificate...")} className="w-full py-3 bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold text-sm text-center rounded-xl hover:bg-emerald-100 transition shadow-sm flex items-center justify-center gap-2">
                        <Download className="w-4 h-4" /> Download Certificate
                      </button>
                    )}

                    {/* Review Section */}
                    <div className="pt-4 mt-2 border-t border-slate-100">
                      {submittedReviews[room.id] ? (
                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center justify-between">
                          <div>
                            <div className="flex gap-0.5 mb-1 text-amber-400">
                              {[1,2,3,4,5].map(star => (
                                <Star key={star} className={`w-3.5 h-3.5 ${star <= submittedReviews[room.id].rating ? 'fill-amber-400' : 'fill-slate-200 text-slate-200'}`} />
                              ))}
                            </div>
                            <p className="text-xs font-bold text-slate-700">Review Submitted</p>
                          </div>
                          <button 
                            onClick={() => handleOpenReview(room)}
                            className="text-xs font-bold text-navy hover:text-sky-600 transition"
                          >
                            Edit Review
                          </button>
                        </div>
                      ) : (
                        <button 
                          onClick={() => handleOpenReview(room)}
                          className="w-full py-3 bg-white border border-slate-200 text-navy font-bold text-sm text-center rounded-xl hover:bg-slate-50 transition shadow-sm flex items-center justify-center gap-2"
                        >
                          <Star className="w-4 h-4 text-amber-400 fill-amber-400" /> Leave Review
                        </button>
                      )}
                    </div>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <div className="bg-white p-12 rounded-2xl border border-slate-200 text-center shadow-sm">
              <div className="w-20 h-20 bg-slate-50 text-slate-300 rounded-full flex items-center justify-center text-3xl mx-auto mb-6">
                <Sparkles className="w-10 h-10" />
              </div>
              <h3 className="font-sora font-bold text-slate-700 text-xl mb-3">You haven't completed any classrooms yet</h3>
              <p className="text-slate-500 text-sm max-w-sm mx-auto">Keep learning! Your successfully completed courses and certificates will appear here.</p>
            </div>
          )
        )}
      </div>
      
      {/* Student Review Modal */}
      <StudentReviewModal 
        isOpen={reviewModalOpen}
        onClose={() => setReviewModalOpen(false)}
        onSubmit={handleSubmitReview}
        classroom={reviewContext}
        existingReview={reviewContext ? submittedReviews[reviewContext.id] : null}
      />
    </div>
  );
}
