...
export default function TeacherClassrooms() {
  ...
  const handleSaveClassroom = async (e) => {
    ...
    try {
      ...
      let msg = err.message || `Failed to ${editingClassroomId ? 'update' : 'create'} classroom`;
      if (err.data && err.data.errors) {
        if (Array.isArray(err.data.errors)) {
          const detail = err.data.errors.map(e => typeof e === 'object' ? `${e.field || ''}: ${e.message || ''}`.trim() : e).join(', ');
          if (detail) {
            msg = `${err.data.message || 'Validation failed'}: ${detail}`;
          }
        }
      }
      setToastMessage(msg);
    ...
    <div className="max-w-6xl mx-auto pb-12 space-y-8 relative">
      ...
      {classrooms.length === 0 ? (
        ...
        <div className="space-y-12">
          ...
          {(statusFilter === 'all' || statusFilter === 'active') && (
            ...
            {classrooms.filter(r => r.status === 'active').length === 0 ? (
              <div className="bg-slate-50 p-8 rounded-xl border border-dashed border-slate-200 text-center">
                <p className="text-slate-500 text-sm font-medium">No active classrooms currently. Click "Create Classroom" to start a new one!</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {classrooms.filter(r => r.status === 'active').map(room => (
                  <div key={room.id} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden transition-all hover:shadow-md hover:border-sky/30">
                    <div className="p-6">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider bg-green-100 text-green-700">
                              Active
                            </span>
                          </div>
                        </div>
          ...
          {/* Completed Classrooms Sub Section */}
            ...
            {classrooms.filter(r => r.status === 'completed').map(room => (
              <div key={room.id} className="bg-slate-50 border border-slate-200 rounded-xl shadow-sm overflow-hidden opacity-80">
                <div className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="font-sora font-bold text-xl text-slate-700 mb-1">{room.name}</h3>
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-[10px] font-bold text-slate-500 bg-slate-200 px-2 py-0.5 rounded uppercase tracking-wider">{room.classLevel || 'N/A'}</span>
                        <span className="text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded uppercase tracking-wider">{room.subject}</span>
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider bg-slate-200 text-slate-700">
                          Completed
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm mb-6">
                    <div className="flex items-center gap-2 text-slate-500 font-medium">
                      <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center shrink-0"><CalendarDays className="w-4 h-4 text-slate-400" /></div>
                      <span className="line-clamp-2">{room.schedule}</span>
                    </div>
                    <div className="flex items-center gap-2 text-slate-500 font-medium">
                      <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center shrink-0"><Users className="w-4 h-4 text-slate-400" /></div>
                      <span>{room.unlimitedStudents ? `${room.enrolled} / Unlimited Seats` : `${room.enrolled} / ${room.capacity} Enrolled`}</span>
                    </div>
                    <div className="flex items-center gap-2 text-slate-500 font-medium">
                    ...
                    <div className="flex items-center gap-2 text-slate-500 font-medium">
                      <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center shrink-0"><IndianRupee className="w-4 h-4 text-slate-400" /></div>
                      <span>₹{room.price} / student</span>
                    </div>
                  </div>
                </div>
                
                <div className="border-t border-slate-200 bg-slate-100/50 p-4 flex justify-between items-center">
                  <Link to={`/teacher/classrooms/${room.id}`} aria-label="View Classroom" title="View Classroom" className="flex items-center gap-2 text-slate-700 hover:text-navy font-bold text-sm transition cursor-pointer">
                    <Eye className="w-4 h-4" /> View Details
                  </Link>
                </div>
              </div>
