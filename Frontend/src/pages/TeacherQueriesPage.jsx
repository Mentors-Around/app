...
import { MessageSquare, Clock, XCircle, CheckCircle, LineChart, FolderOpen, Search, ArrowRight, Archive, Check, AlertCircle } from 'lucide-react';
...
const TeacherQueriesPage = () => {
  ...
  const calculateExpiration = (dateString) => {
    if (!dateString) return { expired: false, text: 'No Date', isWarning: false };
    const submissionDate = new Date(dateString);
    const expirationDate = new Date(submissionDate.getTime() + 24 * 60 * 60 * 1000);
    const now = new Date();
    
    if (now >= expirationDate) {
      return { expired: true, text: 'Expired', isWarning: false };
    }
    ...
    if (hours < 1) {
      return { expired: false, text: `⚠️ Less than 1 hour remaining`, isWarning: true };
    }

    return { expired: false, text: `⏳ ${hours}h ${mins}m remaining`, isWarning: false };
  };

  const [queries, setQueries] = useState(() => {
    let combinedData = [];
    const savedClassroom = localStorage.getItem('trueed_classroom_queries');
    if (savedClassroom) combinedData = [...JSON.parse(savedClassroom)];
    else combinedData = [...teacherQueriesData];

    let hasChanges = false;
    const processedQueries = combinedData.map(q => {
      const timestamp = q.createdAt || q.date;
      if (q.status === 'pending' || q.status === 'pending_review') {
        ...
        if (expired) {
          ...
          return { 
            ...
            events: [...(q.events || []), { id: `e_${Date.now()}`, type: 'system_action', actionType: 'teacher_rejected', timestamp: new Date().toISOString(), content: 'Query was automatically closed because the teacher did not respond within 24 hours.' }]
          };
        }
      } else if (q.status === 'approved_waiting_payment') {
        const timer = getPaymentTimer(q.paymentDeadline);
        if (timer.expired) {
          hasChanges = true;
          return { 
            ...q, 
            status: 'approval_expired', 
            events: [...(q.events || []), { id: `e_${Date.now()}`, type: 'system_action', actionType: 'teacher_rejected', timestamp: new Date().toISOString(), content: 'Approval expired. Payment deadline missed and seat released.' }]
          };
        }
      }
    ...
    if (hasChanges || !savedClassroom) {
      localStorage.setItem('trueed_classroom_queries', JSON.stringify(processedQueries));
    }
  ...
  useEffect(() => {
    ...
    if (savedClassrooms) setTeacherClassrooms(JSON.parse(savedClassrooms));
  }, []);

  useEffect(() => {
    localStorage.setItem('trueed_classroom_queries', JSON.stringify(queries));
  }, [queries]);
  ...
  const handleAccept = () => {
    ...
    if (query) {
      const { sessionStarted, paymentDeadline: deadline } = calculatePaymentDeadline(query.classroomId);
      if (sessionStarted) {
        showToast('Error: This classroom session has already started.');
        setAcceptModalOpen(false);
        return;
      }
      newStatus = 'approved_waiting_payment';
      paymentDeadline = deadline;
      actionTakenStr = 'approved';
      newEvent.actionType = 'teacher_approved';
      newEvent.content = responseText || 'Your classroom request has been approved. Please complete payment to confirm your enrollment.';
    }
    ...
    setAcceptModalOpen(false);
    showToast(newStatus === 'approved_waiting_payment' ? 'Student Approved. Waiting for Payment.' : 'Student Enrolled Successfully');
  };
  ...
  const handleReject = () => {
    ...
    setQueries(queries.map(q => q.id === activeQueryId ? {
      ...
      status: 'rejected',
      events: [...(q.events || []), {
        id: `e_${Date.now()}`, type: 'system_action', actionType: 'teacher_rejected', timestamp: new Date().toISOString(),
        content: `Teacher rejected your request. Reason: ${rejectReason}`
      }]
    } : q));
