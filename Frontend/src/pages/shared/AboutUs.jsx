...
const AboutUs = () => {
  ...
  return (
    <div>
      {/* Hero */}
      <div className="bg-gradient-to-br from-navy to-navy-light text-white py-20 px-6 text-center">
        <div className="max-w-[800px] mx-auto">
          <h1 className="font-sora text-3xl md:text-5xl font-extrabold mb-6 leading-tight">Connecting every Indian student with the right teacher.</h1>
          <p className="text-white/70 text-base md:text-lg max-w-2xl mx-auto">We are on a mission to democratize quality education by providing a transparent, safe, and efficient platform for private tutoring across India.</p>
        </div>
      </div>
      ...
      {/* How it works */}
      <div className="py-16 px-6 max-w-[1100px] mx-auto text-center">
      ...
      <div className="bg-navy text-white py-16 px-6">
        <div className="max-w-[1100px] mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          <div><p className="font-sora text-3xl md:text-4xl font-bold text-amber mb-2">500+</p><p className="text-sm text-white/70">Verified Teachers</p></div>
          <div><p className="font-sora text-3xl md:text-4xl font-bold text-amber mb-2">2000+</p><p className="text-sm text-white/70">Happy Students</p></div>
          <div><p className="font-sora text-3xl md:text-4xl font-bold text-amber mb-2">20+</p><p className="text-sm text-white/70">Cities Active</p></div>
          <div><p className="font-sora text-3xl md:text-4xl font-bold text-amber mb-2">4.8</p><p className="text-sm text-white/70">Average Rating</p></div>
        </div>
      </div>
      ...
      <div className="py-20 px-6 max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="font-sora text-3xl font-extrabold text-navy tracking-tight mb-4">Meet the Team</h2>
          <p className="text-base md:text-lg text-slate-500 font-medium">Building India's most trusted student–teacher learning platform.</p>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            { name: "Rohit Kumar", role: "Founder", initials: "RK" },
            { name: "Sugayan Singh", role: "Co-Founder", initials: "SS" },
            { name: "Akanksha", role: "Co-Founder", initials: "AK" },
            { name: "Hari Prasad L", role: "Co-Founder", initials: "HP" }
          ].map((f, i) => (
            <div key={i} className="text-center bg-white p-10 rounded-[20px] shadow-brand border border-slate-100 hover:-translate-y-2 transition-all duration-300 flex flex-col items-center justify-center h-full">
              <div className="w-24 h-24 bg-gradient-to-br from-navy to-sky text-white rounded-full flex items-center justify-center text-3xl font-bold mb-6 shadow-sm border-4 border-white">
                {f.initials}
              </div>
              <h3 className="font-sora font-bold text-navy text-[20px] mb-1">{f.name}</h3>
              <p className="text-[14px] text-[#0F2B5B] font-medium opacity-80">{f.role}</p>
            </div>
          ))}
        </div>
