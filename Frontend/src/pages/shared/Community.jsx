...
const Community = () => {
  ...
  return (
    <div className="bg-cream/30 min-h-screen font-inter">
      {/* Hero */}
      ...
      <div className="max-w-[1100px] mx-auto py-12 px-6 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
        ...
        <div className="space-y-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => alert('New discussions are locked to verified teachers.')} className="text-sm font-semibold text-sky hover:underline">New Post</button>
          </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="bg-white p-4 rounded-brand shadow-brand border border-slate-100 flex gap-4 hover:shadow-brand transition cursor-pointer">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                <div key={r.title} className="bg-white p-4 rounded-brand border border-slate-200 text-center hover:border-sky transition cursor-pointer">
                  ...
                  <button onClick={() => alert('Download details will be emailed to verified educators.')} className="text-xs font-semibold text-sky bg-sky/10 px-3 py-1.5 rounded-full w-full">Download</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-navy to-navy-light text-white p-5 rounded-brand shadow-brand-lg relative overflow-hidden">
            ...
            <button onClick={() => alert('Registered! Check your email for details.')} className="w-full py-2 bg-white text-navy rounded-lg text-sm font-semibold hover:bg-cream transition relative z-10">Register Now</button>
          </div>
          ...
          <div className="bg-white p-5 rounded-brand border border-slate-200">
            ...
            <div className="space-y-2">
              {['Bangalore Educators', 'Delhi NCR Tutors', 'Mumbai Teaching Hub', 'All India Network'].map(g => (
                <button onClick={() => alert('Invites are sent upon profile approval.')} key={g} className="w-full flex items-center justify-between p-2 rounded-lg hover:bg-[#25D366]/10 text-left transition group">
