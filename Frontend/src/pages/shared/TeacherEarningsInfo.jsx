  ...
  return (
    <div className="font-inter">
      ...
      <div className="bg-navy text-white py-20 px-6 text-center">
        <h1 className="font-sora text-3xl md:text-5xl font-extrabold mb-4">Earn on Your Own Terms</h1>
        <p className="text-white/70 text-base md:text-lg max-w-xl mx-auto mb-8">Set your own rates, choose your hours, and keep 85% of what you earn. Teaching has never been more rewarding.</p>
      ...
      <div className="max-w-[1100px] mx-auto py-16 px-6">
        ...
        {/* Earnings Calculator */}
        <section className="bg-white rounded-brand shadow-brand p-8 md:p-12 mb-20 border border-slate-100 max-w-3xl mx-auto">
          <h2 className="font-sora text-2xl font-bold text-navy text-center mb-8">Estimate Your Earnings</h2>
          
          <div className="space-y-8 mb-10">
            <div>
              <div className="flex justify-between mb-2 text-sm">
                <label className="font-bold text-navy">Hourly Rate (₹)</label>
                <span className="font-bold text-sky">₹{rate}/hr</span>
              </div>
              <input type="range" min="200" max="2000" step="50" value={rate} onChange={e => setRate(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-sky" />
            </div>
            <div>
              <div className="flex justify-between mb-2 text-sm">
                <label className="font-bold text-navy">Sessions per week</label>
                <span className="font-bold text-sky">{sessions} sessions</span>
              </div>
              <input type="range" min="1" max="40" value={sessions} onChange={e => setSessions(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-sky" />
            </div>
          </div>
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-cream p-8 rounded-brand text-center">
              ...
              <p className="text-xs text-muted leading-relaxed">You keep exactly 85% of your hourly rate. The 15% covers platform maintenance, marketing, and payment gateway fees.</p>
            </div>
            <div className="bg-cream p-8 rounded-brand text-center">
              ...
              <p className="text-xs text-muted leading-relaxed">Earnings are automatically transferred to your bank account or UPI ID every Tuesday.</p>
            </div>
            <div className="bg-cream p-8 rounded-brand text-center">
              <i className="fa-solid fa-file-invoice text-3xl text-navy mb-4" />
              <h3 className="font-bold text-navy mb-2">Tax Compliant</h3>
              <p className="text-xs text-muted leading-relaxed">Download automated invoice summaries and TDS certificates directly from your dashboard.</p>
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={t.name} className="border border-slate-200 p-6 rounded-brand flex items-start gap-4 hover:shadow-brand transition bg-white">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-emerald-600">{t.earn}</p>
                </div>
              </div>
            ))}
          </div>
