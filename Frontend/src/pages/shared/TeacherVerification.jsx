  ...
  return (
    <div className="font-inter">
      ...
      <div className="bg-navy text-white py-20 px-6 text-center">
        <h1 className="font-sora text-3xl md:text-5xl font-extrabold mb-4">Get Your Verified Badge</h1>
        <p className="text-white/70 text-base md:text-lg max-w-xl mx-auto mb-8">Build trust with students instantly. A verified badge shows you are a qualified, safe, and professional educator.</p>
        <Link to="/teacher/apply" className="inline-block py-3 px-8 bg-amber text-navy rounded-xl font-bold hover:bg-amber-hover transition">
          Start Verification
        </Link>
      </div>
      ...
      <div className="max-w-[1100px] mx-auto py-16 px-6 space-y-24">
        ...
        <section className="text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-white p-8 rounded-brand shadow-brand border border-slate-100">
              <i className="fa-solid fa-eye text-3xl text-sky mb-4" />
              <h3 className="font-bold text-navy mb-2">More Visibility</h3>
              <p className="text-xs text-muted">Verified profiles rank higher in student search results and get featured on the homepage.</p>
            </div>
            <div className="bg-white p-8 rounded-brand shadow-brand border border-slate-100">
              <i className="fa-solid fa-shield-heart text-3xl text-success mb-4" />
              <h3 className="font-bold text-navy mb-2">Higher Trust</h3>
              <p className="text-xs text-muted">Parents and students are 4x more likely to book a teacher who has a verified badge.</p>
            </div>
            <div className="bg-white p-8 rounded-brand shadow-brand border border-slate-100">
              ...
              <p className="text-xs text-muted">Command higher hourly rates by proving your qualifications and teaching experience.</p>
            </div>
          </div>
        </section>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center relative">
              ...
              {[
                ...
                <div key={s.step} className="relative z-10 flex flex-col items-center mb-8 md:mb-0 bg-cream md:px-4">
                  ...
                  <h4 className="font-bold text-navy text-xs">{s.title}</h4>
                </div>
              ))}
            </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <ul className="space-y-4">
              <li className="flex items-center gap-3 text-navy font-medium text-sm"><i className="fa-solid fa-circle-check text-success" /> Government ID (Aadhaar / PAN)</li>
              <li className="flex items-center gap-3 text-navy font-medium text-sm"><i className="fa-solid fa-circle-check text-success" /> Highest Degree Certificate</li>
              <li className="flex items-center gap-3 text-navy font-medium text-sm"><i className="fa-solid fa-circle-check text-success" /> Experience Proof (Optional but recommended)</li>
              <li className="flex items-center gap-3 text-navy font-medium text-sm"><i className="fa-solid fa-circle-check text-success" /> Clear Profile Photo</li>
            </ul>
          </div>
          <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
            <div className="aspect-video bg-slate-200 rounded-lg flex items-center justify-center text-muted flex-col gap-2">
              ...
              <span className="text-xs font-semibold">Watch: How to upload documents correctly</span>
            </div>
          </div>
        </section>
          ...
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <div key={i} className="border border-slate-200 rounded-lg overflow-hidden bg-white shadow-sm">
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-center justify-between p-4 text-left hover:bg-slate-50 transition">
                  <span className="font-bold text-navy text-sm">{faq.q}</span>
                  <i className={`fa-solid fa-chevron-down text-muted text-xs transition-transform ${openFaq === i ? 'rotate-180' : ''}`} />
                </button>
                {openFaq === i && (
                  <div className="p-4 bg-slate-50 border-t border-slate-200 text-xs text-muted leading-relaxed">
                    {faq.a}
                  </div>
                )}
              </div>
