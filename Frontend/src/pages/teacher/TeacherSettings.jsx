...
import {
  Mail, Globe, LifeBuoy, AlertCircle, Shield, CheckCircle, ChevronDown,
  ChevronUp, CreditCard, Monitor, User, Bug, Phone, Laptop, Clock, Download,
  AlertTriangle, Bell, Eye, MapPin, Loader2, EyeOff
} from 'lucide-react';
...
export default function TeacherSettings() {
  ...
  const [privacy, setPrivacy] = useState({
    publicTeacherProfile: true,
    showRatingOnProfile: true,
  ...
  const supportCategories = [
    { name: 'Account & KYC Profile', icon: <User className="w-5 h-5 text-sky-500" /> },
    { name: 'Classroom Management', icon: <Monitor className="w-5 h-5 text-purple-500" /> },
    { name: 'Payouts & Earnings', icon: <CreditCard className="w-5 h-5 text-emerald-500" /> },
    { name: 'Queries & Tokens', icon: <LifeBuoy className="w-5 h-5 text-blue-500" /> },
    { name: 'Technical Support', icon: <Bug className="w-5 h-5 text-rose-500" /> }
  ];

  const faqs = [
    { q: 'How do I create a classroom?', a: 'To create a classroom, go to the classrooms tab from your sidebar, click "Create Classroom," fill in details such as Title, Mode (Live vs pre-recorded), Schedule, and Fees. Once submitted, it will be reviewable by students.' },
    { q: 'How do payouts work?', a: 'Teacher earnings from student enrollments are added to your pending balance. Once classroom sessions are completed and verified, your balance can be withdrawn to your linked bank account from the Payouts dashboard.' },
    { q: 'What is KYC verification?', a: 'All teachers are required to upload a government ID and degree proof during onboarding. This KYC is verified by TrueEd administrators before your classrooms can accept enrollments.' },
    { q: 'How do I answer student doubts?', a: 'Inside the classroom details page, go to the "Doubts" tab. Here you can see a list of student doubts. Click on a doubt to answer it. Answers can include text and downloadable reference materials.' },
    { q: 'What are query tokens?', a: 'Query tokens are paid to you by students who want to consult you before purchasing a course. Earning query tokens helps boost your rating and overall payout metrics.' }
  ];
  ...
  const getKycBadge = (status) => {
    switch (status) {
      case 'approved':
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-700 rounded-full border border-emerald-100 text-xs font-bold"><CheckCircle className="w-3.5 h-3.5" /> KYC Verified</span>;
      case 'rejected':
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-rose-50 text-rose-700 rounded-full border border-rose-100 text-xs font-bold">KYC Rejected</span>;
      case 'under_review':
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 text-amber-700 rounded-full border border-amber-100 text-xs font-bold">KYC Under Review</span>;
      default:
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-50 text-slate-700 rounded-full border border-slate-100 text-xs font-bold">KYC Pending</span>;
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto pb-12 space-y-8 animate-fadeIn relative px-4 md:px-0">
      <div>
        <h1 className="font-sora text-3xl font-extrabold text-navy mb-2">Teacher Settings & Support</h1>
        <p className="text-slate-500 font-medium">Manage your teacher portal settings and platform FAQs.</p>
      </div>
        ...
        <div className="md:col-span-3">
                ...
                {Object.keys(notifications).map((key) => (
                  <div key={key} className="flex items-center justify-between py-3 border-b border-slate-50 last:border-0">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 font-medium">Receive alerts about {key.toLowerCase()}</p>
                    </div>
                    <button
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8 space-y-6">
              ...
              <div className="space-y-6 max-w-xl">
                <div className="flex items-center justify-between py-3 border-b border-slate-100">
                  <span className="font-semibold text-navy text-sm">KYC Status</span>
          ...
          {activeTab === 'data' && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8">
              ...
              <p className="text-sm font-medium text-slate-600 mb-6">Request a zip file contains your teaching logs, payouts histories, and classroom details.</p>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-200">
                <h2 className="font-sora font-bold text-xl text-slate-900 mb-6 flex items-center gap-2">
                  <AlertCircle className="w-5 h-5 text-amber-500" /> Teacher FAQs
                </h2>
          ...
          {activeTab === 'danger' && (
            <div className="bg-white rounded-2xl shadow-sm border border-error/20 p-6 md:p-8">
              ...
              <p className="text-sm font-medium text-slate-600 mb-8">Once you delete your account, all your teacher profile data, classrooms, and payout history will be permanently lost.</p>
